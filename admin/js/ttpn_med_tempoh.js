...
function tableTempoh(){
    ...
    var settings = {
        "url": host + "api_media/public/tempohListAll",
        "method": "GET",
        "timeout": 0,
      };
    ...
    $.ajax(settings).done(function (response) {
        ...
        $.each(response.data, function (i, field) {
            ...
            list.push({
                id: field.id_tempoh, tempoh: tempoh + " Hari", bil: bil++,
                status_rekod: '<label class="adomx-switch-2 success "><input type="checkbox" id="tempoh_sistem" class="form-control mb-20" '+checked+' onclick="del_rekod(\''+field.id_tempoh+'\')"> <i class="lever"></i> <span id="text_statusrekod'+field.id_tempoh+'" class="badge '+ badge +'">'+text_statusrekod+'</span></label>', 
...
function del_rekod(i){
    ...
    $.ajax(settings).done(function (response) {
        ...
        if (result.data.statusrekod == 1)   {
            $('#text_statusrekod'+result.data.id_tempoh).text('Aktif').removeClass("badge-danger").addClass("badge-success");
        } else  {
            $('#text_statusrekod'+result.data.id_tempoh).text('Tidak Aktif').removeClass("badge-success").addClass("badge-danger");
        }
