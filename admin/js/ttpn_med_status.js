...
function tableStatus() {
  ...
  $.ajax(settings).done(function (response) {
    ...
    $.each(response.data, function (i, field) {
      ...
      list.push({
        id: field.id_status,
        kod_status: field.kod_status,
        nama_status: field.nama_status,
        bil: bil++,
        status:
...
function del_rekod(i) {
  ...
  $.ajax(settings).done(function (response) {
    ...
    if (result.data.statusrekod == 1) {
      $("#text_statusrekod" + result.data.id_status)
        .text("Aktif")
    ...
    saveLog(
      ...
      "Change Record Status for [id_status = " +
        id_status +
          ...
          swal({
            ...
            function (dismiss) {
              if (dismiss === "timer") {
                saveLog(
                  window.sessionStorage.id,
                  "Update Data for [id_status = " +
                    upt_id +
                    "]" +
                    upt_nama_status +
                    " at Tetapan Status Media.",
                  window.sessionStorage.browser
                );
                $("#update-medstatus").modal("hide");
                tableStatus();
