    ...
    if (!confirmed) {
        ...
        check_users(noic,function(){
            if(obj_users.success && (obj_users.data.users_intan == '1')){
                ...
                $("#nama_pegawai").html('<i class="fa fa-user"></i> ' +obj_users.data.nama);
                $("#FK_users").val(noic);
...
function check_users(noic,returnValue){
    var settings = {
        "url": host+"api_pentadbir/public/usersgovsIntan/"+noic,
        "method": "GET",
        "timeout": 0,
      };
    $.ajax(settings).done(function (response) {
    ...
    $.ajax(settings).done(function (response) {
        ...
        $.each(response.data, function (i, field) {   
            ...
            list.push({
                ...
                "upt_btn":  '<button class="button button-box button-sm button-primary" onclick="loadDataCapaian(\'' + i + '\')" data-ui-toggle-class="zoom" data-ui-target="#animate"><i class="ti-pencil-alt"></i></button> ' 
...
function tablePentadbir(){
    ...
    var settings = {
        "url": host + "api_pentadbir/public/usersListPentadbir",
        "method": "GET",
        "timeout": 0,
      };
    ...
    $.ajax(settings).done(function (response) {
        ...
        $.each(response.data, function (i, field) {   
            ...
            list.push({
                id: field.id_users, nama: field.nama, emel: field.emel, no_kad_pengenalan: field.no_kad_pengenalan, 
                notel: field.notel, jenis_pengguna: field.jenis_pengguna, nama_peranan: field.nama_peranan, 
                bil: bil++, data_intan: field.nama_kampus + '/ ' + field.nama_kluster + '/ ' + field.nama_subkluster, 
                status_rekod: '<label class="adomx-switch-2 success "><input type="checkbox" id="status_sistem" class="form-control mb-20" '+checked+' onclick="del_rekod(\''+field.id_capaian+'\',\''+field.statusrekod_capaian+'\')"> <i class="lever"></i> <span id="text_statusrekod'+field.id_capaian+'" class="badge '+ badge +'">'+text_statusrekod+'</span></label>', 
                "upt_btn":  '<button class="button button-box button-sm button-primary" onclick="loadDataCapaian(\'' + i + '\')" data-ui-toggle-class="zoom" data-ui-target="#animate"><i class="ti-pencil-alt"></i></button> ' 
                            // '<button class="button button-box button-sm button-danger" title="Hapus" onclick="del_rekod(\''+field.id_users+'\')"><i class="ti-trash"></i>'
            });
        });
...
$("#updateCapaian").on('submit',function(e){
    ...
    if (!confirmed) {
        ...
        swal({
            ...
            // alert(FK_subkluster)
            var settings = {
                "url": host+"api_pentadbir/public/capaianUpdate",
                "method": "POST",
                "timeout": 0,
                "processData": false,
                "mimeType": "multipart/form-data",
            ...
            $.ajax(settings).done(function (response) {
                ...
                if (!result.success) {
                    ...
                    saveLog(window.sessionStorage.id,"Update Data [FK_peranan: "+ FK_peranan +"], [FK_users: "+ FK_users +"],  at Tetapan Peranan & Capaian.",window.sessionStorage.browser);
                    $("#update-capaian").modal("hide");
                    tablePentadbir();
                }                
            });
...
function del_rekod(i,status){
    ...
    console.log(id)
    var settings = {
    "url": host+"api_pentadbir/public/capaianDelete",
        "method": "POST",
        "timeout": 0,
        "processData": false,
        "mimeType": "multipart/form-data",
...
function del_rekod_users(i,status){
    ...
    console.log(id)
    var settings = {
    "url": host+"api_pentadbir/public/usersDelete",
        "method": "POST",
        "timeout": 0,
        "processData": false,
        "mimeType": "multipart/form-data",
...
$('#FK_kampus_add').change(function(){
    var settings = {
        "url": host+"api_public/public/klusters/" + $('#FK_kampus_add').val(),
        "method": "GET",
        "timeout": 0
    };
...
$('#FK_kluster_add').change(function(){
    ...
    var settings = {
        "url": host+"api_public/public/subklusters/" + $('#FK_kluster_add').val(),
        "method": "GET",
        "timeout": 0
      };
        $.ajax(settings).done(function (response) {
...
$('#FK_subkluster_add').change(function(){
    //Dropdown Unit List
    var settings = {
        "url": host+"api_public/public/units/" + $('#FK_kluster_add').val() + "/" + $('#FK_subkluster_add').val(),
        "method": "GET",
        "timeout": 0
    };
...
function subklusterListNonKiara()   {
    ...
    //Dropdown Subkluster List
    var settings = {
        "url": host+"api_public/public/subklusters/" + FK_kluster,
        "method": "GET",
        "timeout": 0
      };
        $.ajax(settings).done(function (response) {
