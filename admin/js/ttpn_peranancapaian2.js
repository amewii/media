    ...
    $.ajax(settings).done(function (response) {
        ...
        $.each(response.data, function (i, field) {
            ...
            list.push({
                ...
                bil: bil++,
                status_rekod: '<label class="adomx-switch-2 success "><input type="checkbox" id="status_sistem" class="form-control mb-20" '+checked+' onclick="del_rekod(\''+field.id_capaian+'\',\''+field.statusrekod_capaian+'\')"> <i class="lever"></i> <span id="text_statusrekod'+i+'" class="badge '+ badge +'">'+text_statusrekod+'</span></label>', 
...
function del_rekod(i,status){
    ...
    swal({
            ...
            $.ajax(settings).done(function (response) {
                ...
                saveLog(window.sessionStorage.id,"Update Data for [id = " + id + "], [FK_users = " + FK_users + "], [FK_modul = " + upt_FK_modul + "] at Tetapan Admin Pengguna.",window.sessionStorage.browser);
                window.location.reload();  
            });
        });
