$(function(){
    ...
    if (window.sessionStorage.content == null)  {
        statBilProgram();
        $("#chart").html('<script src="assets/js/plugins/chartjs/chartjs.active.js"></script>');
    }
...
$.ajax(settings).done(function (response) {
    ...
    $.each(response.data, function (i, field) {
        ...
        } else if((field.flag_vip == '0') && (window.sessionStorage.id_peranan == '2') && (window.sessionStorage.FK_kluster == field.FK_kluster))  {
            bil++;
            document.getElementById("badge").classList.add("badge");
            data_notication = ' <li>'+
                                    '<a id="view_permohonan" onclick="viewPermohonan()">'+
...
$("#ubahkatalaluan").click(function () {
    ...
    saveLog(window.sessionStorage.id,"Ubah Katalaluan.",window.sessionStorage.browser);
...
function statBilProgram(){   
    ...
    $.ajax(settings).done(function (response) {
        ...
        $("#program_media").html(((program_media/jumProgram)*100).toFixed(2) + "% Mempunyai Media");
        $("#progress_bar").width(((program_media/jumProgram)*100).toFixed(2) + "%");
    });  
}
