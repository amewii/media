$(function(){
                    ...
                    listnotification = '<li>'+
                                            '<div class="timeline-panel">'+
                                                '<div class="media-body">'+
                                                    '<h6 class="mb-1">'+item.nama_program+'<br>Status: '+ item.nama_status +'</h6>'+
                                                    '<small class="d-block">Sah Sehingga: '+t_luput.getDate()+'/'+(t_luput.getMonth() + 1)+'/'+t_luput.getFullYear()+'</small>'+
                                                '</div>'+
                                            '</div>'+
                                        '</li>';
    ...
    if ((window.sessionStorage.content) == 'halamanutama')  {
        ...
        $("#data_load").html("");
        $("#permohonan").hide();
        $("#page_title").html('<li class="breadcrumb-item active"><a href="javascript:void(0)">Ubah Katalaluan</a></li>');
...
function loadHalamanUtama(){
    ...
    var gallery_tab = '<div class="default-tab mt-60">'+
...
function loadSenaraiProgramBergambar(varAPI,varAppend){
    ...
    $.ajax(settings).done(function (response) {
        ...
        $.each(response.data, function (i, field) {
            ...
            if(varAppend == 'image')    {
                flag = 1;
                thumbnail = '<img class="img-fluid" src="../api_asdcm/api_media/public/uploads/'+ imgsrc +'" alt="">';
            } 
            else {
                flag = 2;
                thumbnail = '<video id="video-element" src="../api_asdcm/api_media/public/uploads/'+ imgsrc +'" controls>'+
                    ...
                    '<div class="row m-b-30">'+
                        ...
                        '<div class="col-md-7 col-xxl-12">'+
                            '<div class="new-arrival-content position-relative">'+
                                ...
                                '<p>Kluster: <span class="item">' + field.nama_kluster + '</span></p>'+
                                '<p>VIP: <span class="item">' + field.FK_vip + '</span></p>'+
                                '<button class="btn btn-info" onclick="detail_media('+field.PK+','+flag+')"><i class="fa fa-view"></i> Lihat</button>'+
                            '</div>'+
                        '</div>'+
                    '</div>'+
                '</div>'+
...
function detail_media(indexs,varType){
    ...
    $.ajax(settings).done(function (response) {
        ...
        $.each(media_path, function (i, field) {
            ...
            if(varType == 1){
                ...
                if(ext[1] != 'mp4' && ext[1] != 'mov'){
                    ...
                    preview = '<label for="'+ field.images +'"><img class="img-fluid"  style="height:225px" src="../api_asdcm/api_media/public/uploads/'+field.images+'" alt=""></label>';
                    
                    data_programs = '<div class="pic-view">'+
                ...
                if(ext[1] == 'mp4' || ext[1] == 'mov'){
                    ...
                    preview = '<label for="'+ field.images +'"><video id="video-element'+field.images+'" height="200">'+
                                    '<source src="../api_asdcm/api_media/public/uploads/'+ field.images +'" type="video/mp4" preload="none" >'+
                                '</video></label>'+
                                '<canvas id="canvas-element'+field.images+'"></canvas>';;

                    data_programs = '<div class="vid-view">'+
                                '<div class="card p-0-7">'+
                                    '<div class="card-body p-0"><input class="" type="checkbox" name="media_selected[]" value="'+ field.images +';'+field.FK_vip+'" id="'+ field.images +'"/>'+
                                        '<div class="new-arrival-product">'+
                                            '<div class="new-arrivals-img-contnent">'+
        ...
        if(count_vid == 1) class_vid_length = 'col-xl-6 col-lg-6 col-md-6 col-sm-6';
        else if(count_vid == 2) class_vid_length = 'col-xl-5 col-lg-6 col-md-6 col-sm-6';
        else if(count_vid > 2) class_vid_length = 'col-xl-3 col-lg-6 col-md-6 col-sm-6';
        $(".vid-view").addClass(class_vid_length);

        let class_pic_length = '';
        let count_pic = $('.pic-view').length;
        if(count_pic == 1) class_pic_length = 'col-xl-6 col-lg-6 col-md-6 col-sm-6';
        else if(count_pic == 2) class_pic_length = 'col-xl-5 col-lg-6 col-md-6 col-sm-6';
        else if(count_pic > 2) class_pic_length = 'col-xl-3 col-lg-6 col-md-6 col-sm-6';
...
function getThumbnail(images){
    data_programs = '<script>'+
                        'var _VIDEO = document.querySelector("#video-element"'+images+'),'+
                        '_CANVAS = document.querySelector("#canvas-element"'+images+');'+
...
function data_program(){
    ...
    $.ajax(settings).done(function (response) {
        $.each(response.data, function (i, field) {
            ...
            t_program = new Date(field.tarikh_program);
            data_programs = '<div class="col-lg-6 col-xl-6">'+
            '<div class="card">'+
                '<div class="card-body">'+
                    '<div class="row m-b-30">'+
                        ...
                        '<div class="col-sm-12 col-md-7 col-xl-7">'+
                            '<div class="new-arrival-content position-relative">'+
                                ...
                                '<p>Kategori Program: <span class="item">' + field.nama_kategori + '</span> </p>'+
                                '<p>Lokasi: <span class="item">' + field.nama_kampus + '</span></p>'+
                                '<button class="btn btn-info" onclick="detail_media('+field.PK+')"><i class="fa fa-view"></i> Lihat</button>'+
                            '</div>'+
                        '</div>'+
                    '</div>'+
                '</div>'+
    ...
    $.ajax(settings).done(function (response) {
        ...
        $.each(response.data, function (i, field) {
            ...
            list.push({
                ...
                bil: bil++,
                "upt_btn":  '<button type="button" class="badge badge-'+button_update+'"loadData(\''+i+'\')"><i class="fa fa-pencil-square-o"></i></button> '+
            ...
            data_programs = '<div class="col-xl-3 col-lg-6 col-md-6 col-sm-6">'+
                                '<div class="card">'+
                                    '<div class="card-body"><input class="" type="checkbox" name="media_selected[]" value="'+ field.images +'" id="'+ field.images +'"/>'+
                                        '<div class="new-arrival-product">'+
                                            '<div class="new-arrivals-img-contnent">'+
                                                '<label for="'+ field.images +'"><img class="img-fluid" src="../api_asdcm/api_media/public/uploads/'+field.images+'" alt=""></label>'+
                                            '</div>'+
                                        '</div>'+
                                    '</div>'+
                                '</div>'+
...
$("#updatepermohonan").on('submit', function (e) {
    ...
    if (!confirmed) {
        ...
        swal({
                ...
                var settings = {
                    "url": host+"api_media/public/permohonanUpdatePemohon",
                    "method": "POST",
                    "timeout": 0,
                    "processData": false,
                    "mimeType": "multipart/form-data",
...
function del_record(){
    ...
    swal({
        ...
        var settings = {
            "url": host+"api_media/public/permohonanCancel",
            "method": "POST",
            "timeout": 0,
            "processData": false,
            "mimeType": "multipart/form-data",
        ...
        $.ajax(settings).done(function (response) {
            ...
            if (!result.success) {
                ...
                swal({
                    ...
                    sessionStorage.content = 'list';
                    saveLog(window.sessionStorage.id,"Cancel Data ["+ nama_program +"] at Permohonan Media.",window.sessionStorage.browser);
                    $("#update-permohonan").modal("hide");                         
                });
...
function listNotification(returnValue)  {
    var settings = {
        "url": host + "api_media/public/permohonanByUsersNotification/"+window.sessionStorage.id,
        "method": "GET",
        "timeout": 0,
    };
