document.getElementById("no_kad_pengenalan_pelajar").value =
...
$("#poskod_rumah_pelajar").blur(function () {
  var settings = {
    url: host + "sysposkod/" + $("#poskod_rumah_pelajar").val(),
    method: "GET",
    timeout: 0,
  };

  $.ajax(settings).done(function (response) {
    document.getElementById("daerah_rumah_pelajar").value =
      response.data.bandar;
    document.getElementById("negeri_rumah_pelajar").value = response.data.nama;
    document.getElementById("negara_rumah_pelajar").value =
      response.data.kodnegara;
  });
});

$("#poskod_sekolah_pelajar").blur(function () {
  var settings = {
    url: host + "sysposkod/" + $("#poskod_sekolah_pelajar").val(),
    method: "GET",
    timeout: 0,
  };

  $.ajax(settings).done(function (response) {
    document.getElementById("daerah_sekolah_pelajar").value =
      response.data.bandar;
    document.getElementById("negeri_sekolah_pelajar").value =
      response.data.nama;
  });
});
...
$.ajax(settings).done(function (response) {
  ...
  $("#FK_negara_lahir_pelajar").append(
    $("<option>", {
      value: "",
      text: "Pilih Negara",
    })
...
$.ajax(settings).done(function (response) {
  ...
  $("#FK_negeri_lahir_pelajar").append(
    $("<option>", {
      value: "",
      text: "Pilih Negeri",
    })
