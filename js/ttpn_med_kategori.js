...
function tableKategori() {
  ...
  var settings = {
    url: host + "kategoriprogramListAll",
    method: "GET",
    timeout: 0,
  };
...
function loadData(indexs) {
  ...
  saveLog(
    ...
    "View Data of [id_kategoriprogram = " +
      data[indexs].id_kategoriprogram +
      "]" +
      data[indexs].nama_kategori +
      " at Tetapan Kategori Media.",
  ...
  if (!confirmed) {
    ...
    swal({
      ...
      let kod_format = $('#checkboxformat [type="checkbox"]:checked')
        ...
        .get();
      let flag_kod_format = $('#checkboxformat [type="checkbox"]:checked')
        .map(function () {
          return this.value;
        })
        .get();
...
$("#update").on("submit", function (e) {
  ...
  if (!confirmed) {
    ...
    swal({
      ...
      var settings = {
        url: host + "kategoriprogramUpdate",
        method: "POST",
        timeout: 0,
        processData: false,
        mimeType: "multipart/form-data",
          ...
          swal({
            ...
            function (dismiss) {
              if (dismiss === "timer") {
                saveLog(
                  window.sessionStorage.id,
                  "Update Data for [id_kategoriprogram = " +
                    upt_id +
                    "]" +
                    upt_nama_kategori +
                    " at Tetapan Kategori Media.",
...
function del_rekod(i) {
  ...
  var settings = {
    url: host + "kategoriprogramDelete",
    method: "POST",
    timeout: 0,
    processData: false,
    mimeType: "multipart/form-data",
  ...
  $.ajax(settings).done(function (response) {
    ...
    if (result.data.statusrekod == 1) {
      $("#text_statusrekod" + result.data.id_kategoriprogram)
        .text("Aktif")
    ...
    saveLog(
      window.sessionStorage.id,
      "Change Record Status for [id_kategoriprogram = " +
        id_kategoriprogram +
        "] at Tetapan Kategori Media.",
      window.sessionStorage.browser
    );
